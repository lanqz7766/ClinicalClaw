...
function renderHero(workspace) {
  ...
  const scoreMap = { watch: 42, alert: 74, urgent: 93 };
...
function renderTrend(workspace) {
  ...
  const circles = points
    .map(
      (point, index) =>
        `<circle cx="${toX(index)}" cy="${toY(point.value)}" r="4.5" fill="#0f6fb8"></circle>`,
    )
    .join("");
  svg.innerHTML = `
    <defs>
      <linearGradient id="trendFill" x1="0%" y1="0%" x2="0%" y2="100%">
        <stop offset="0%" stop-color="rgba(15, 111, 184, 0.28)" />
        <stop offset="100%" stop-color="rgba(15, 111, 184, 0.02)" />
      </linearGradient>
    </defs>
    <path d="${area}" fill="url(#trendFill)"></path>
    <path d="${path}" fill="none" stroke="#0f6fb8" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></path>
    ${circles}
  `;
...
function renderActions(workspace) {
  ...
  document.getElementById("chat-log").innerHTML = `
    <div class="chat-bubble">
      ...
      <p>${workspace.risk_label} case for ${workspace.workflow_title}. Current disposition: ${workspace.disposition.replace(/_/g, " ")}.</p>
    </div>
  `;
}
...
async function rerunCase() {
  ...
  const workspace = await fetchJson("/api/findings/rerun", {
    method: "POST",
...
async function askQuestion(event) {
  ...
  const response = await fetchJson("/api/findings/explain", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function reviewCase(action) {
  ...
  const workspace = await fetchJson("/api/findings/review", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  document.querySelectorAll(".prompt-chip").forEach((button) => {
    button.addEventListener("click", async () => {
      const response = await fetchJson("/api/findings/explain", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ case_id: state.caseId, question: button.dataset.question }),
      });
      renderWorkspace(response.workspace);
      document.getElementById("chat-log").innerHTML += `
        <div class="chat-bubble">
